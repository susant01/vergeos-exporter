...
- Tenant Metrics:
  - Tenant Status
  - Total Tenant on cluster
  - Total Node on Tenant
  - Tenant Node Status
  - Tenant allocation and utilization of CPU/RAM/Storage per Node
...
1. Create a systemd service file at `/etc/systemd/system/vergeos-exporter.service`:
...
1. Copy the vergeos-exporter executable to this directory:

```powershell
copy vergeos-exporter.exe "C:\Program Files\vergeos-exporter"
```

1. Install the service using NSSM (run Command Prompt as Administrator):
